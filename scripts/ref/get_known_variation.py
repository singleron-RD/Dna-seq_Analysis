...
if type == "all":
    ...
    raise ValueError(f"Unsupported type {type} (only all, somatic, structural_variations are allowed)")

species_filename = species if release >= 91 else species.capitalize()

urls = [
    f"ftp://ftp.ensembl.org/pub/{branch}release-{release}/variation/vcf/{species}/{species_filename}{suffix}.{ext}"
